	...
	const fetchAToZItems = async ( search = '' ) => {
		try {
			...
			const data = await apiFetch( {
				path: `/custom/v1/a-to-z-posts${ query }`,
			} );
			setAToZItems( data );
		} catch ( error ) {
			...
			<div className="a-to-z-index-container wp-block-group alignfull has-global-padding is-layout-constrained wp-block-group-is-layout-constrained">
				<div className="a-to-z-index alignwide">
					...
					{ isLoading ? (
						...
						<div
							className={ `a-to-z-index-alphabet${
								isSticky ? ' a-to-z-index-alphabet--fixed' : ''
							}` }
							ref={ alphabetRef }
						>
							{ aToZItems.map( ( group ) => (
										...
										<ul className="a-to-z-index-section-list">
											{ [ ...Array( 5 ) ].map(
												( e, i ) => (
													<li
														key={ i }
														className="a-to-z-index-section-list-item placeholder-glow"
													>
														<a
						...
						<div className="a-to-z-index-sections">
							{ aToZItems.map( ( group ) => (
								<div
									...
									ref={ sectionRefs.current[ group.letter ] }
								>
									<div className="a-to-z-index-section-drop-cap">
					...
					{ isBackToVisible && (
						<button
							className="a-to-z-index-back-to"
