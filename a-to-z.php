...
/**
 * Include Import Settings Page and Plugin Update Checker
 */
require_once plugin_dir_path( __FILE__ ) . 'admin/import-settings.php';
require_once plugin_dir_path( __FILE__ ) . 'admin/plugin-update-checker.php';
...
/**
 * Register A to Z custom post type
 */
...
function utk_a_to_z_register_taxes() {
	...
	register_taxonomy( 'a_to_z_category', array( 'a_to_z' ), $args );
}
 ...
 *
 * @param string $request Request made to the API.
 */
function get_a_to_z_posts_by_letter( $request ) {
	...
	if ( ! empty( $search ) ) {
		...
		foreach ( $search_posts as $post ) {
			...
			$categories = wp_get_post_terms( $post->ID, 'a_to_z_category', array( 'fields' => 'names' ) );

			$grouped_posts[ $group_key ][] = array(
				'ID'         => $post->ID,
				'title'      => html_entity_decode( $post->post_title, ENT_QUOTES | ENT_HTML5, 'UTF-8' ),
				'url'        => $a_to_z_url ? $a_to_z_url : '',
	...
	// Non-alpha group.
	$non_alpha_posts = $wpdb->get_results(
		$wpdb->prepare(
			"
        SELECT ID, post_title 
        FROM $wpdb->posts 
